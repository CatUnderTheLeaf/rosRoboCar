...
class _AckermannCtrlr(object):
    ...
    def twist_cmd_cb(self, cmd_vel):
        """Twist driving command callback

        :Parameters:
          cmd_vel : geometry_msgs.msg.Twist
            Twist driving command.
        """
        self._last_cmd_time = rospy.get_time()
        with self._ackermann_cmd_lock:
            self._steer_ang = cmd_vel.angular.z
            self._steer_ang_vel = 1
            self._speed = cmd_vel.linear.x #/0.1
            self._accel = 1
